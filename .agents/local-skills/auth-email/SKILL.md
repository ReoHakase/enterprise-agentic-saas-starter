...
- magic link、email verification、organization invitationは `@enterprise-agentic-saas/email/runtime` の `createRuntimeEmailSender` で統一する。workerd exportはCloudflare `EMAIL` binding、default exportはMailpit/console/noopを使う。既定providerはdevelopment=`mailpit`、test=`noop`、production=`cloudflare`とし、productionのMailpit/consoleはfail-closedにする。API dev supervisorがMailpit wrapperのtoken付きlocal sessionからdirect loopback `MAILPIT_URL`を注入するため、auth側で固定URLや別resolverを持たない。
...
- `multiSessionClient.setActive`を使う全account切替は、consoleのAgent Shell有無や招待landingなどの呼出元に依存させず、共有account switch componentで旧session cookieのまま`POST /agent/context/revoke`を成功させる。失敗時はaccountを切り替えない。shell callbackはdraft risk確認とlocal cleanup専用であり、security revokeの条件にしない。旧identityのin-flight queryは`cancelQueries()`で停止するが、この段階でcacheをclearしない。先にclearするとmounted queryが旧cookieで即再取得し、旧tenant dataを再投入する。`setActive`成功後、新cookieの状態で全QueryClient cacheをcancel/clearし、Agent stream/upload/thread/draft/form/Blob URLをclearする。最後は通常`/dashboard`、招待中は検証済みの招待pathへfull-document navigationし、Next router cacheと旧Server Component propsも破棄する。account identity境界を`router.replace()`と`router.refresh()`だけで済ませない。旧accountのactive organizationや`agentThread`を新accountへ引き継がない。
...
- active organizationを変更するDB transactionはAgent session context epochも1だけrotationし、旧connection ticket、grant、run、action、resume ticket、approval policy、asset leaseを同じtransactionで失効する。現行migrationはsession update triggerを正本にするため、application helperから同じepochを二重incrementしない。
...
- invitation landingは未ログインuserを即sign-in redirectせず、招待URLを`redirectTo`へ保持したsign-up/sign-inを表示する。招待詳細はBetter Auth `get-invitation`がactive sessionのemail一致を確認した場合だけ表示し、403の別email sessionにはacceptを出さずmulti-session switch/add accountを促す。account切替はcookie変更後に認証cacheを消してNext/React treeをhard reloadするが、招待中は検証済みの同一origin `/invitations/:id`を明示的な戻り先として保持する。外部origin、protocol-relative path、backslash、control文字は`/dashboard`へfail closedする。recipient emailをpublic context APIへ複製しない。
...
- Passkey sign-in、sign-outのようにdocument reloadなしでidentityが変わる境界では、TanStack Queryの全queryをcancelしてQueryClient cacheをclearする。multi-session切替だけは旧identityでcancel、`setActive`成功後にclearする二段階にし、旧cookieでの自動再取得raceを防ぐ。console queryだけのinvalidateではissue/comment等の別keyに前accountのtenant dataが残るため不十分。
