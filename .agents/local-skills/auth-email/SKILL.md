...
- Agent Shellを持つconsoleで`multiSessionClient.setActive`を呼ぶ前に、旧session cookieで`POST /agent/context/revoke`を成功させる。失敗時はaccountを切り替えない。成功後はidentity-scoped TanStack Queryをcancel/clearしてから`setActive`し、Agent stream/upload/thread/draft/form/Blob URLをclearして`router.refresh()`する。旧accountのactive organizationや`agentThread`を新accountへ引き継がない。
