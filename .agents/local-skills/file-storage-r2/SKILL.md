...
- fileを削除するtransactionでは先に`storageObjectId`、`keyVersion`、`objectKey`をsnapshotし、file delete triggerがfile claimを外した直後に`releaseDeletedFileStorageObjectsInTransaction`を呼ぶ。v2はrevision fence付きexact cleanupへ移し、v1のowner-prefix/exact cleanupは互換backstopとして残す。
