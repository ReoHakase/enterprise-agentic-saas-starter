...
- Agent journeyは標準mock harnessのfake eventだけで実provider接続まで成功扱いにしない。外部課金を伴う明示的な`test:e2e:agent` suiteで、実API Worker、private Agent Worker、Mastra `product-agent`、OpenRouter `qwen/qwen3.6-flash`を起動する。自然Web検索、履歴・Issueを使う一般化検索、Issue link、自動title、Yes/Noとwrite、画像、mention/page context、thinking/tool、context/usage、過去approval reloadを実stackで確認する。
...
- paid Agent sentinelはuser prompt自体にも含まれるため、全message articleの文字列一致だけでstream成功扱いにしない。先に`POST /agent/chat`が200かつ`text/event-stream`で応答したことを確認し、`Issue agent`と識別できるassistant article内だけでsentinelを待ってからAPI canonical assistant historyを照合する。
