...
- Agent journeyは標準mock harnessのfake eventだけで実provider接続まで成功扱いにしない。外部課金を伴う明示的な`test:e2e:agent` suiteで、実API Worker、private Agent Worker、Mastra `product-agent` / 専用title Agent、OpenRouter `qwen/qwen3.6-flash`を起動する。renameを依頼しない自動title、自然Web検索、履歴・Issueを使う一般化検索、Issue link、Ask alwaysのYes/No、Full access、Tiptap mention、thinking/tool/context、過去approval reloadを実stackで確認する。画像は1 message内でupload→公開Web検索→画像付きIssue作成まで完了させ、tool inputとDBのIssue/file/claimをassertする。
...
- paid Agent sentinelはuser prompt自体にも含まれるため、全message articleの文字列一致だけでstream成功扱いにしない。先に`POST /agent/chat`が200かつ`text/event-stream`で応答したことを確認し、`aria-label=Agent response`のassistant article内だけでsentinelを待ってからAPI canonical assistant historyを照合する。完了/reload後にtransient statusが残らずassistant IDが重複しないことも確認する。
- `test:eval:agent`はrelease scenarioごとに独立状態で3回実行し、tool call、DB state、stream part、安全境界の3/3成功を要求する。回答文面一致やtest retryで確率的失敗を隠さない。詳細は`docs/agent/testing.md`を正本にする。
- paid Agentの各turn待機は製品provider上限2分に30秒のstream/UI反映猶予を加えた150秒にする。完了後のerror alertも不在であることを検証し、Playwright retryは0のまま維持する。timeoutしたrunはinfrastructure failureとして合格に数えない。
