...
- `senders/mailpit.ts`: local development専用。接続先はloopbackまたは`localhost` / `*.localhost`のHTTP(S)だけを許可し、`POST /api/v1/send`へ`From`、`To`、`Subject`、`Text`、任意`HTML`、template `Tags`だけを渡す。Workersが拒否する`redirect: "error"`は使わず`manual`にしてredirectを追わない。`renderProps`、raw response、provider raw errorを保持せず、timeoutとsanitized error codeを持つ。
...
- `@enterprise-agentic-saas/email/config`のresolverは未指定providerをdevelopment=`mailpit`、test=`noop`、production=`cloudflare`へ揃える。workerdはPortlessの開発CAを信頼しないため、Mailpit wrapperが起動ごとのtoken付きsessionへdirect loopback URLを公開し、API dev supervisorがreadiness確認後にWranglerへ注入する。browser UIはworktree-awareなPortless HTTPSを維持する。resolverの固定URLはpackage単体起動用fallbackに限定する。`resolveEmailFrom`はlocal/testで未設定のときだけ配送不能な`noreply@example.test`を返し、本番は`undefined`を返してenv validationを失敗させる。`apps/api`と`packages/auth`で独自fallbackを二重実装せず、envだけのimportでtemplate runtimeを読み込まない。
