...
- Agent shellのconversationは初期末尾表示と96px以内の自動追従を持ち、message、stream、画像、周辺resizeを`ResizeObserver`からanimation frame単位で処理する。上へ離れた履歴閲覧中は追従せず、末尾復帰で再開する。2 user turn以上では右端にturn minimapを出し、user prompt、直後のassistant本文、画像・context・tool件数のlocal previewとclick/Enter移動を提供する。native scrollbar、messageの`article`、conversationの`aria-live`を維持し、専用Agent pageへminimapを出さない。
