...
- Agent UIも同じconsole layoutで`AgentShellProvider`と`AgentShell`を一度だけmountし、専用Agent pageごとにchat runtimeを作らない。desktopは右側resizable pane（360〜720px、既定460px）、mobileはdrawer外ownerのfull-screen Sheetにし、route navigation中もthreadとcomposerを維持する。shell open、pane幅、短命composerはJotai、thread/message/approval/usageはserver state、selected threadはACL確認付きnuqsへ置く。
- Agent header、未保存thread draft、inline approval、thinking/tool trace、mention、context/usage meter、40vh composer、hotkeyの表示契約は`docs/agent/chat-ui.md`を正本にする。`@tanstack/react-hotkeys`を使い、IME、upload、modal scopeをtestする。model textからIssue linkやauthorizationを作らない。
...
- Bunの`patchedDependencies`にはOpenNext 1.20.1の文字列形式`exports`をcopy対象と誤認する問題と、Base UI 1.4.1のmiddleware option重複warningの最小patchがある。依存更新時はupstream修正を確認し、patchを惰性で持ち越さず、削除可否を`bun run check`と`build:cloudflare`で判定する。
