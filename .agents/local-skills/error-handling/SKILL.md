...
- HTTP responseへ返してよいのはpublic code、`AppError.publicMessage`、runtimeでsanitizeしたpublic context、request ID、許可済み`fieldErrors`だけ。
- `AppError.publicMessage`はapplication側が公開可と明示するtrust marker。診断用かつ変更可能な`Error.message`、provider/DB/Valibotのraw messageは、`AppError`へ包んだ後もresponseやUIへ渡さない。
...
- WebはEden responseをWeb-local Valibot schemaでparseし、成功した`ConsoleApiError`のpublic messageだけを公開可能として扱う。任意のJavaScript/Valibot/network errorの`message`は表示しない。
- unknown error、`internal_error`、不正responseは操作別の固定fallbackへ変換する。5xxでは復旧案内と安全なrequest IDをreferenceとして表示できるが、raw detailを付けない。
- API errorの表示ownerはmutation/formなど一箇所に決め、Query cacheのglobal handlerとcomponent handlerから同じ失敗を二重toastしない。
- `fieldErrors`は一致するfieldのinput直下だけに表示し、入力変更時に解消する。`aria-invalid`、説明要素のID、`aria-describedby`を同期し、fieldに紐づかない失敗はform alertまたはtoastへ出す。
- TanStack Queryのdefault error policyは`QueryClient`生成時に設定する。mounted observerの後から`useEffect`でcache defaultを変更しない。
...
- Elysia/Valibotのrequest validationは400 `validation_error`へ正規化し、安全な`fieldErrors`だけを許可する。OpenAPI responseも400に揃え、runtimeが返さない422をdocumentしない。
- response validationはserver contract違反なので500 `internal_error`へ丸め、内部field pathを公開せずSentryへcaptureする。request validationと同じ400として扱わない。
- error responseの`requestId`が必須で、OpenAPI schemaとruntime responseが一致することを確認する。
