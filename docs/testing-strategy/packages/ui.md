...
```text
lib
  → React、component、patternへ依存しない

hooks
  → lib
  → component、patternへ依存しない
...
| **UI複合パターンブラウザー統合テスト (UI4)** | 統合                | <ul><li>複数primitiveを組み合わせたdialog、drawer、menu、combobox、form patternを確認する</li><li>portal、focus trap、roving tabindex、outside click、Escape、scroll lockを確認する</li><li>複数component間のstateとcallbackの協調を確認する</li><li>Storybook storyで表現しにくい場合だけ`.browser.test.tsx`を使う</li></ul>                                        | 実browser、複数UI component、portal、CSS、browser API              | application state、API、QueryClient、auth、tenant     | `packages/ui/src/patterns/**`、複合component、限定的な`*.browser.test.tsx`                  | Vitest Browser ModeまたはStorybook Vitest addon | 中から遅い | なし                 | 必要な範囲     |
...
UI4はUI package内の複数primitive協調へ限定します。
...
```json
{
  "scripts": {
    ...
    "test:browser": "vitest run --project=storybook-light --project=storybook-dark --project=browser"
  }
}
```
