...
| **DBスキーマ・制約統合テスト (DB2)**           | 統合                | <ul><li>current schemaへ直接rowを作り、FK、unique、partial unique、check、cascadeが働くことを確認する</li><li>tenant composite FK、outbox ownership、idempotency、privacy columnの不変条件を確認する</li><li>indexとtriggerが期待どおり存在し、query plan上必要なindexが欠落していないことを確認する</li><li>`pragma foreign_key_check`が空になることを確認する</li></ul>                                                                                                  | current Drizzle schema、実libSQL、constraint、trigger、index                   | remote Turso、API repository、external provider     | `packages/db/src/schema/**`、schema-specific integration test、constraint fixture                  | Vitest + in-memory libSQL                     | 速い       | なし                 | 厚くする   |
...
- active super adminがゼロになる更新を拒否する
- 同じidempotency keyを重複作成できない
- outbox rowがownerを失わない
- cascadeの範囲が過大でない
