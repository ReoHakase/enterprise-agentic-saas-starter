...
| -------------------------------- | ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------- | ----------------------------------------------------- | -------------------------------------------------------------------------------- | ------------------------------------------ | ------------------ | -------------------- | ---------------------- |
| **Agent中核単体テスト G1**       | 単体                | <ul><li>abort、disconnect、provider error、timeoutを別状態へ分類する</li><li>Stopではsubmission IDを保持せず、disconnect/errorだけretry identityを保持する</li><li>run全体timeout、tool side effect後retry禁止を境界値で確認する</li><li>model provider開始前とstream完了時だけlivenessを再検証する</li><li>resource ID、thread ID、model route、tool allowlist、reasoning policyを確認する</li><li>usage normalisation、safe error、secret scrubを確認する</li></ul>                                                                                                                                                                                                                                                                                  | pure function、Valibot schema、policy                                                    | clock、ID、pricing table                              | `apps/agent/src/mastra/core/**`、policy、usage、error                            | Vitest Node                                | 極めて速い         | なし                 | 非常に多い             |
...
| **Agent制御面統合テスト G4**     | 統合                | <ul><li>API thread registryとAgent Memoryが同じthread IDを使う</li><li>API認可後だけlist/recallでき、archive後はAgent dataが残っていても拒否する</li><li>`startChatRun`がticket消費、run、quotaを同じトランザクションで確定する</li><li>非公開`port`が18操作で旧互換操作を持たない</li><li>explicit cancel、Stop直後の次run、Agent abort、expiryが一つのterminal stateへ収束する</li><li>stream途中の失効後は最終response、Memory保存、次のmodel/tool stepを拒否する</li><li>`finalizeRun`がusageとterminal状態を1回で確定し、usage失敗でもrunningを残さない</li><li>Web検索の拒否をquota予約より先に行い、認可と冪等予約を1操作で確認する</li><li>current permission再検証、attachment transaction、Service Bindingの本番contractを確認する</li></ul> | 実API app、実Agent runtime、Application libSQL、Agent libSQL、Service Binding相当adapter | modelはscripted、search/R2はcontrolled fake           | `apps/agent/**`、`apps/api/src/modules/agent/**`                                 | Vitest + Elysia + libSQL                   | 中から遅い         | なし                 | 必要な範囲で厚くする   |
...
- 可視のreasoning deltaを独自の無通信timerへ結び付けず、270秒のrun全体timerは延長しない
- livenessはprovider開始前とstream完了時の2回だけ確認し、断片数では増えない
...
- Web検索query完全一致と、API認可前にproviderを呼ばないこと
- private hostname除外
...
stream途中の失効では最終受理、Memory保存、次のmodel/tool step、業務副作用が拒否されることを検査します。
完了再検証までに現在のstream断片がbrowserへ届くことは許容し、回収を要求しません。
...
OAuth MCPの本番相当transportはMCP OAuth journeyの独立gateで確認し、現在のAgent有料E2には
含めません。

E2は製品と同じ`openrouter-gpt-5.6-luna-xhigh`を使います。標準の`test:e2e:full`は、
`agent-canary-web-search-source`、`agent-canary-private-issue-read`、
`agent-canary-approved-issue-write`の3本を各1回実行します。画像入力は有料E2の対象にしません。
`@diagnostic-qwen`と`PAID_E2E_DIAGNOSTIC`による除外は廃止します。

system側のallowlist、exact asset選択、遅延bind、過去画像reuse、approval前の非mutation、resume後の
promotionは引き続き決定的なE1とAPI契約で必須化します。reasoning `xhigh`の送信はtransport契約テストで
固定します。E2は使用量APIで`openrouter`、`openai/gpt-5.6-luna`、実行数、出力tokenの記録を確認し、
reasoning tokenを検証値にしません。API本文は安全な真偽値と件数へ変換し、プロンプト、モデル出力、
