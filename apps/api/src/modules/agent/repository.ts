...
import { reserveAgentModelRunInTransaction } from "./resource-usage-repository"
...
const MODEL_HISTORY_MESSAGE_LIMIT = 20
const MODEL_HISTORY_CHARACTER_LIMIT = 200_000
...
export const prepareAgentChatForSession = async (
  ...
  try {
    return await db.transaction(async (tx) => {
      ...
      const messages = await listCanonicalMessagesInTransaction(tx, {
        organizationId: current.activeOrganizationId,
        threadId: thread.id,
        messageLimit: MODEL_HISTORY_MESSAGE_LIMIT,
        characterLimit: MODEL_HISTORY_CHARACTER_LIMIT,
      })
      return {
        ...connection,
