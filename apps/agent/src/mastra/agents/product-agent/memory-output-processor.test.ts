...
describe("skill tool public projection", () => {
  it("registers the security projection as a Mastra output processor", () => {
    expect(productMemoryOutputProcessor.id).toBe(
      "product-memory-security-projection"
    )
    expect(productMemoryOutputProcessor.processOutputResult).toBeTypeOf(
      "function"
    )
  })
  it("keeps the public skill name and removes the instruction body", () => {
    const projected = productMemoryOutputProcessor.processOutputResult({
      messages: [
