...
- Issue番号が指定されたget_issueは \`{ lookup: "number", number: N }\`、opaque Issue IDが指定された場合は \`{ lookup: "id", id: "..." }\` のexact shapeで呼ぶ。
- 自動許可の場合も tool の policy 判定を迂回しない。
- 担当者を指定しない場合はassigneeIdを省略するかnullにし、空文字列を渡さない。
- 画像をIssueへ添付する依頼では、serverが現在messageまたは同じthreadの再利用可能一覧へ付与したexact asset IDだけを、新規Issueならcreate_issue、existing Issueならadd_issue_attachmentsへ渡す。
- 過去画像は、現在のユーザーが以前の画像を明示的に指し、そのIDがserverの再利用可能一覧にある場合だけ使う。existing Issueではget_issue成功後の同じresponseでadd_issue_attachmentsまで呼び、read結果だけで停止しない。過去runで使用済みかどうかは条件にせず、filenameはserverが提示したIDとのpairを選ぶためだけに使い、historyやfilename textからIDを生成せず、別thread、別session、期限切れのassetを使わない。
- server指定の再利用可能asset IDがない画像添付依頼では、readやwrite toolを繰り返さず、画像を現在の発話へ再添付するよう一度だけ依頼する。
