...
const createMemoryBoundaryFixture = async (ticket: string) => {
  ...
  const dependencies: Parameters<typeof handleMemoryHistory>[2] = {
    ...
    createControlPlane: () => ({
      consumeConnectionTicket: async ({
        ...
        if (!tickets.delete(presented) || requested !== threadId) {
  ...
  return {
    composition,
    dependencies,
    environment: { AGENT_INTERNAL_API: {} },
    threadId,
  }
}

describe("App registryからAgent Memoryへの境界", () => {
  it("運用上のMemory境界失敗時に固定codeを記録する", async () => {
    const composition = createAgentRuntimeComposition({
      AGENT_RUNS_ENABLED: "1",
      AGENT_VISION_ENABLED: "0",
      AGENT_WRITES_ENABLED: "0",
  ...
  it("認可済み接続で公開履歴だけを返す", async () => {
    const { composition, dependencies, environment, threadId } =
      await createMemoryBoundaryFixture("ticket_history")
  ...
  it("認可済みregistry内のthreadを一覧へ返す", async () => {
    const { composition, dependencies, environment, threadId } =
      await createMemoryBoundaryFixture("ticket_threads")
