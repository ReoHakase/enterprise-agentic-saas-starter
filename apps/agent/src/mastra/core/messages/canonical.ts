...
const isContextBudgetLevel = (
  value: unknown
): value is "critical" | "normal" | "notice" | "warning" =>
  value === "normal" ||
  value === "notice" ||
  value === "warning" ||
  value === "critical"
...
const sanitizeContextBudget = (
  data: Record<string, unknown>
): AgentCanonicalMessagePart | undefined => {
  ...
  if (
    ...
    !isContextBudgetLevel(data.level)
  ) {
    return undefined
  }
  return {
...
const sanitizeThreadTitle = (
  data: Record<string, unknown>
): AgentCanonicalMessagePart | undefined => {
  const { renamed, threadId, title } = data
  if (
  ...
  switch (part.type) {
    case "text":
      return typeof part.text === "string"
        ? { type: "text", text: part.text.slice(0, 50_000) }
        : undefined
    // 実行中statusはtransient streamだけで扱い、canonical履歴へ保存しない。
    case "data-activity":
      return undefined
    case "data-context-budget":
      return isRecord(part.data) ? sanitizeContextBudget(part.data) : undefined
    case "data-thread-title":
      return isRecord(part.data) ? sanitizeThreadTitle(part.data) : undefined
    case "reasoning":
      return typeof part.text === "string"
        ? { type: "reasoning", text: part.text.slice(0, 20_000) }
        : undefined
    case "step-start":
      return { type: "step-start" }
    case "source-url":
      return sanitizeSource(part)
    default:
