...
const scrubAgentSentryEvent = <T extends SentryEventLike>(event: T): T => {
  delete event.user
  delete event.extra
...
const scrubAgentSentryLog = <T extends SentryLogLike>(log: T): T => {
  log.attributes = safeTags(log.attributes)
  log.message = "Agent runtime log"
  return log
}

const scrubAgentSentrySpan = <T extends SentrySpanLike>(span: T): T => {
  span.data = {}
  delete span.links
...
const filterAgentSentryIntegrations = <T extends { name: string }>(
  integrations: T[]
): T[] =>
  integrations.filter(
    ({ name }) =>
