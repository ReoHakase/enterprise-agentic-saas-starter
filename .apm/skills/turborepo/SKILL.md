...
1. **If dependencies ARE declared** (e.g., `"@enterprise-agentic-saas/types": "workspace:*"` in package.json), remove the `prebuild` script. Turbo's `dependsOn: ["^build"]` handles this automatically.
...
2. **If dependencies are NOT declared**, the `prebuild` exists because `^build` won't trigger without a dependency relationship. The fix is to:
   - Add the dependency to package.json: `"@enterprise-agentic-saas/types": "workspace:*"`
...
{
  "dependencies": {
    "@enterprise-agentic-saas/types": "workspace:*",
    "@enterprise-agentic-saas/utils": "workspace:*"
  },
  "scripts": {
    "build": "next build"
  }
...
// WRONG - root turbo.json with many package-specific overrides
{
  "tasks": {
    "test": { "dependsOn": ["build"] },
    "@enterprise-agentic-saas/web#test": { "outputs": ["coverage/**"] },
    "@enterprise-agentic-saas/api#test": { "outputs": ["coverage/**"] },
    "@enterprise-agentic-saas/utils#test": { "outputs": [] },
    "@enterprise-agentic-saas/cli#test": { "outputs": [] },
    "@enterprise-agentic-saas/core#test": { "outputs": [] }
  }
}
