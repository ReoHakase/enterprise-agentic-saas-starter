...
const insertPendingUpdateAction = (
...
const approveAction = (
  ...
  client.execute({
    ...
    args: [`approve-${id}`, now + 1, now + 1, id],
  })

const insertResumeTicket = (
  client: ReturnType<typeof createClient>,
  input: { actionId: string; id: string; now: number; tokenHash: string }
) =>
  client.execute({
...
describe("Agent action runtimeのschema", () => {
  ...
  beforeEach(async () => {
    ...
    now = await insertActionFixture(client)
  })
  ...
  it("Issue更新時にrevisionを1だけ進める", async () => {
    ...
    await expect(
      client.execute("update issues set revision = 4 where id = 'action-issue'")
    ).rejects.toThrow(/issue_revision_must_increment_by_one/i)
  })

  it("runへclient message idを必須にする", async () => {
    await expect(
      client.execute({
    ...
    ).rejects.toThrow(/agent_run_required_identifier_missing|check constraint/i)
  })

  it("未承認actionを成功状態へ進めない", async () => {
    await insertPendingUpdateAction(client, now)
  ...
  it("成功済みactionを別の終端状態へ変更しない", async () => {
    await insertPendingUpdateAction(client, now)

    await approveAction(client, "update-action", now)
    await client.execute({
      sql: "update issues set title = ?,revision = ?,updated_at = ? where id = ? and revision = ?",
      args: ["Agent update", 2, now + 2, "action-issue", 1],
    })
    await client.execute({
    ...
    ).rejects.toThrow(/agent_action_invalid_state_transition|check constraint/i)
  })

  it("actionごとに有効なresume ticketを1件へ限定する", async () => {
    await insertPendingUpdateAction(client, now)
    await approveAction(client, "update-action", now)
    await insertResumeTicket(client, {
    ...
    await expect(
      insertResumeTicket(client, {
        actionId: "update-action",
        id: "resume-ticket-duplicate",
        now,
  ...
  it("消費済みresume ticketの再利用を拒否する", async () => {
    await insertPendingUpdateAction(client, now)
    await approveAction(client, "update-action", now)
    await insertResumeTicket(client, {
      actionId: "update-action",
  ...
  it("sessionのorganization変更時にcontextを進めて一時状態を失効させる", async () => {
    await insertPendingUpdateAction(client, now)
    await approveAction(client, "update-action", now)
    await insertResumeTicket(client, {
      actionId: "update-action",
    ...
    await insertPendingUpdateAction(client, now + 7, "switch-action")
    await client.execute({
    ...
    const [context, action, run, policy, ticket] = await Promise.all([
      ...
      client.execute(
        "select revoked_at as revokedAt from agent_resume_tickets where id = 'resume-ticket'"
      ),
    ])
...
const insertResourceUsageBuckets = (
  client: ReturnType<typeof createClient>,
  now: number
) =>
  client.batch([
...
const promoteActionAsset = async (
  client: ReturnType<typeof createClient>,
  now: number
) => {
  ...
  await approveAction(client, "create-action", now + 1)
  await client.batch([
    {
      sql: "insert into issues(id,organization_id,number,title,creator_id,created_at,updated_at) values(?,?,?,?,?,?,?)",
      args: [
...
describe("Agent action runtimeのresourceとasset不変条件", () => {
  let client: ReturnType<typeof createClient>
  let now: number
  ...
  it("同じ使用量操作をbucketごとに1回だけ反映する", async () => {
    await insertResourceUsageBuckets(client, now)
    const insertOperation = (bucketId: string) =>
      client.execute({
        sql: "insert into agent_resource_usage_operations(operation_id,organization_id,bucket_id,delta,created_at) values(?,?,?,?,?)",
        args: ["same-operation", "action-org", bucketId, 2, now],
      })

    await insertOperation("organization-bucket")
    await insertOperation("user-bucket")
    await expect(insertOperation("organization-bucket")).rejects.toThrow(
      /unique/i
    )

    const buckets = await client.execute(
      "select id,count from agent_resource_usage_buckets where id in ('organization-bucket','user-bucket') order by id"
    )
    expect(buckets.rows).toMatchObject([
      { count: 2, id: "organization-bucket" },
      { count: 2, id: "user-bucket" },
    ])
  })

  it("上限を超える使用量操作を記録せず拒否する", async () => {
    await insertResourceUsageBuckets(client, now)

    await expect(
      client.execute({
        sql: "insert into agent_resource_usage_operations(operation_id,organization_id,bucket_id,delta,created_at) values(?,?,?,?,?)",
        args: ["overflow", "action-org", "user-bucket", 5, now],
      })
    ).rejects.toThrow(/check constraint/i)

    const [bucket, operation] = await Promise.all([
      client.execute(
        "select count from agent_resource_usage_buckets where id = 'user-bucket'"
      ),
      client.execute(
        "select operation_id from agent_resource_usage_operations where operation_id = 'overflow'"
      ),
    ])
    expect(bucket.rows).toMatchObject([{ count: 0 }])
    expect(operation.rows).toHaveLength(0)
  })

  it("反映済みの使用量操作を不変にする", async () => {
    await insertResourceUsageBuckets(client, now)
    await client.execute({
      sql: "insert into agent_resource_usage_operations(operation_id,organization_id,bucket_id,delta,created_at) values(?,?,?,?,?)",
      args: ["consume", "action-org", "user-bucket", 2, now],
    })
    await client.execute({
      sql: "insert into agent_resource_usage_operations(operation_id,organization_id,bucket_id,delta,created_at) values(?,?,?,?,?)",
      args: ["release", "action-org", "user-bucket", -2, now + 1],
    })
  ...
  it("規定statement順で1つのstaged assetをIssue fileへ昇格する", async () => {
    await promoteActionAsset(client, now)
  ...
  it("昇格後のIssue file削除時に元assetとclaimを破棄する", async () => {
    await promoteActionAsset(client, now)

    await client.execute("delete from files where id = 'promoted-file'")

    const [asset, claims] = await Promise.all([
      client.execute(
  ...
  it("storage object削除時にaction assetのsnapshot参照を解除する", async () => {
    await promoteActionAsset(client, now)
    await client.execute("delete from files where id = 'promoted-file'")
    await client.execute(
